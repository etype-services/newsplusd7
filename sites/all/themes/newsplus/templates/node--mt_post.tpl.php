...
<article id="node-<?php print $node->nid; ?>" class="<?php print $classes; ?> clearfix"<?php print $attributes; ?>>
    ...
    <div id="printArea" class="node-content-wrapper clearfix">
                    ...
                    <div id="affix">
                        ...
                        <?php if ($share_links) { ?>
                            ...
                            <div class="share-links">
                                ...
                                <ul>
                                    <li>
                                        <a href="https://www.facebook.com/sharer/sharer.php?u=<?php print urlencode(url($path, array('absolute' => TRUE))) ?>&t=<?php print urlencode($title) ?>"
                                           onclick="window.open(this.href, 'facebook-share','width=580,height=296');return false;"><span><i
                                                        class="fab fa-facebook"></i></span></a></li>
                                    <li>
                                        <a href="https://plus.google.com/share?url=<?php print urlencode(url($path, array('absolute' => TRUE))) ?>"
                                           onclick="window.open(this.href, 'google-plus-share', 'width=490,height=530');return false;"><span><i
                                                        class="fab fa-google-plus"></i></span></a></li>
                                    <li>
                                        <a href="http://twitter.com/share?text=<?php print urlencode($title) ?>&url=<?php print urlencode(url($path, array('absolute' => TRUE))) ?>"
                                ...
                                <?php if ($print_button) { ?>
                                    <div class="print">
                                        <i class="fas fa-print"></i> <a href="javascript:window.print()"
                                                                        class="print-button"> <?php print t(' Print') ?></a>
                                    </div>
                                <?php } ?>
                                <?php if ($font_resize) { ?>
        ...
        <!-- node-content -->
        <div class="node-content clearfix <?php if (!($share_links || $print_button || $font_resize || $display_submitted || $post_progress || $reading_time) || (!$page)) {
            print ' ' . 'full-width';
        } ?> ">
            <header>
                <?php print render($title_prefix); ?>
                <?php if (!$page) { ?>
                    <h2 <?php print $title_attributes; ?> class="title"><a
                ...
                <?php if (isset($content['field_mt_post_categories']) || $display_submitted || (module_exists('comment') && ($node->comment == COMMENT_NODE_OPEN || ($node->comment == COMMENT_NODE_CLOSED && $node->comment_count > 0)))) { ?>
                    <div class="node-info">
                        ...
                        <?php if (isset($content['field_mt_post_categories'])) { ?>
                            <div class="node-info-item"><i
                                        class="fas fa-tags"></i> <?php print t('Posted in: '); ?><?php print render($content['field_mt_post_categories']); ?>
                            </div>
                        <?php } ?>
                        <?php if (module_exists('comment') && ($node->comment == COMMENT_NODE_OPEN || ($node->comment == COMMENT_NODE_CLOSED && $node->comment_count > 0))) { ?>
                            <div class="node-info-item"><i class="fas fa-comments"></i> <?php print $comment_count; ?>
                                comments
                            </div>
                        <?php } ?>
                    </div>
